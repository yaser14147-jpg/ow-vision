...
def download_file(url, local_path):
    ...
    try:
        ...
        r = requests.get(url_with_cache_bust, stream=True, timeout=20) 
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if float(remote['version']) > float(local['version']):
                ...
                print("------------------------------------------")
                print("[+] Starting CORE System Sync...")
                ...
                # 2. Update AI Model (If missing or major sync)
                if not os.path.exists(LOCAL_MODEL_PATH) or float(remote['version']) >= 6.2:
                     download_file(MODEL_URL, LOCAL_MODEL_PATH)
                
                # 3. Update CORE Presets only
