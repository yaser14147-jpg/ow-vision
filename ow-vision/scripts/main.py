...
# --- [v21.0 ELITE COMPACT - ZERO SOUND] ---
...
class ClassicAHKUI:
    def __init__(self, root):
        ...
        self.root.geometry("340x630") # Returned to original width
        ...
        self.current_lang = "AR"; self.visualize_active = False; self.aimbot_running = False; self.app_version = "21.0"; self.process = None
        ...
        # Power Presets (Space Efficient Grid)
        ...
        for i, key in enumerate(self.keys):
            row_frame = ttk.Frame(inputs_frame); row_frame.pack(fill=tk.X, pady=1)
            lbl = ttk.Label(row_frame, text=fields_text[i][0]); lbl.pack(side=tk.LEFT)
            self.label_names.append(lbl)
            ent = ttk.Entry(row_frame, width=12, justify="center"); ent.pack(side=tk.RIGHT)
            self.entries[key] = ent
            desc = ttk.Label(inputs_frame, text=fields_text[i][1], font=("Segoe UI", 7), foreground="#777777")
            desc.pack(anchor="w", pady=(0, 3)); self.label_descs.append(desc)

        # Actions
        btns_frame = ttk.Frame(main_frame); btns_frame.pack(fill=tk.X, pady=(10, 0))
        self.btn_aim_on = ttk.Button(btns_frame, text="Aimbot ON", command=self.start_ai); self.btn_aim_on.grid(row=0, column=0, padx=2, pady=2, sticky="ew")
        self.btn_aim_off = ttk.Button(btns_frame, text="Aimbot OFF", command=self.stop_ai, state=tk.DISABLED); self.btn_aim_off.grid(row=0, column=1, padx=2, pady=2, sticky="ew")
        self.btn_load = ttk.Button(btns_frame, text="Load Profile", command=self.load_config); self.btn_load.grid(row=1, column=0, padx=2, pady=2, sticky="ew")
        self.btn_save = ttk.Button(btns_frame, text="Save Profile", command=self.save_config); self.btn_save.grid(row=1, column=1, padx=2, pady=2, sticky="ew")
        ...
        # Bottom Bar
        bottom_info_frame = ttk.Frame(main_frame); bottom_info_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=(10, 0))
